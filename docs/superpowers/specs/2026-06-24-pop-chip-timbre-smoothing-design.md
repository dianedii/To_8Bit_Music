...
**状态：** 已实现。实际最终方案与本文档原始计划有显著偏离，详见第 7 节。

---

## 7. 实际实现与本文档的偏离（2026-06-27 更新）

后续迭代根据用户提供的 `goals/` 参考样例重新设计了音色，最终实现与本文档原始方案差异较大：

1. **不再暴露波形 / chip_mix 控件**
   - UI 中移除了波形选择、芯片音色占比滑块，仅保留整体音量与输出格式。
   - 所有音色参数（波形、层配比、衰减、EQ、压缩）由 `src/pop_synthesizer.py` 内部固定，以贴近参考样例听感。

2. **转录方式变更**
   - 流行模式最终使用 `piano_transcription_inference` 进行钢琴音符转录，而非本文档计划的 pYIN + 候选线评分。
   - 保留了 `src/pop_melody.py` 中的 pYIN 路径作为 `synthesize_pop_chip()` 的备用入口。

3. **合成器架构变更**
   - 采用多层事件驱动合成：三角波低音 + 方波主旋律 + 方波 +12/+24 亮度层 + 高音区方波。
   - 每个音符施加指数衰减包络，限制最大音符长度，抑制长音“嗡嗡”拖尾。
   - 新增基于 onset 的 8-bit 噪声鼓点。
   - 总线处理包含高通、高频搁架提升、压缩、峰值归一化、软限幅。

4. **新增视频输入支持**
   - `src/worker.py` 支持 MP4 / M4V / MOV / MKV / AVI / FLV / WebM 输入，自动提取音频为 MP3 后进入转换流程。

5. **README 已同步更新**
   - 新的 README 不再描述波形选择与 chip_mix，改为说明内部自动配器与视频输入能力。
